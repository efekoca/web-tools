<?php
$addStat = $connect->prepare("UPDATE stats SET estimated = estimated + 1 LIMIT 1");
$addStat->execute();
?>
    <!DOCTYPE html>
    <html lang="tr">
    <head>
        <title><?php echo($settings["title"]); ?> - Estimated Reading Time</title>
                ...
                <div class="col-11">
                    <div class="card shadow-sm">
                        <div class="card-body px-4 py-5 px-md-5">
                            ...
                            <p class="text-muted card-text">Calculate quickly average estimated reading time for specified content by one click.</p>
                            <?php if(!empty($_POST["text"])){ ?>
                                <div class="alert alert-dark" role="alert">
                                    <?php echo("Average estimated reading time: <b>" . estimatedReadingTime($_POST["text"]) . "</b>"); ?>
                                </div>
                            <?php } ?>
                            <form method="post">
                                ...
                                <button style="margin-top: 20px;" class="btn btn-dark shadow" type="submit">Calculate</button>
                            </form>
                        </div>
                    </div>
                </div>
